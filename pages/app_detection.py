...
try:
    ...
    data_revision = DATA_FILE.stat().st_mtime_ns
...
def _detect_jump_events(series, time_series, span, data_revision):
    """
    ...
    """
    # data_revision 仅作为 Streamlit 缓存键；live_data.csv 更新后强制重新检测。
    _ = data_revision
...
if has_curve_stream:
    for code in valid_codes:
        ...
        jump_events = _detect_jump_events(series, df_ts['Time'], engineering_span, data_revision)
...
with col_table_head: st.markdown("#### 📋 系统全量测点监控及实时值清单")
with col_btn_right:
...
status_list, realtime_vals, alarm_aggregates = [], [], []
...
for _, row in active_system_df.iterrows():
    ...
    if not stream_ok:
        ...
        row_jump_events = _detect_jump_events(actual_curve, df_ts['Time'], row_span, data_revision)
